...
public class PlayerListener implements Listener {
        ...
        if (e.getClickedBlock().getType() == Material.QUARTZ_STAIRS) {
            ...
            Bukkit.getScheduler().runTask(CookieAddon.getIns(), () -> {
                if (stand.isValid()) {
                    ...
                    Hologram hologram = HologramsAPI.createHologram(CookieAddon.ins, e.getPlayer().getLocation().add(0, 1.8, 0));
    ...
    public void onEnterGame(EnterGameBoxEvent event) {
        if (!CookieAddon.getChars().containsKey(event.getPlayer().getName())) {
            event.setCancelled(true);
